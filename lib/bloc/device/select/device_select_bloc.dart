...
class DeviceSelectBloc extends Bloc<DeviceSelectEvent, DeviceSelectState> {
  ...
  Future<void> _onStarted(
    ...
    try {
      ...
      final tables = await _getTables(devices);
  ...
  Future<Map<String, Table>> _getTables(List<Device> devices) async {
    final tables = <String, Table>{};
    for (final device in devices) {
      if (device.tableId == null) continue;

      tables[device.id] = await _tableRepo.fetchTable(device.tableId!);
    }
